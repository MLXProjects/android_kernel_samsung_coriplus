...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	if (dhd->pub.busstate == DHD_BUS_DOWN || dhd->pub.hang_was_sent) {
		DHD_ERROR(("%s: xmit rejected pub.up=%d busstate=%d mmc_suspend=%d\n",
			__FUNCTION__, dhd->pub.up, dhd->pub.busstate, dhd_mmc_suspend));
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	printf("Broadcom Dongle Host Driver: register interface [%s]"
		...
		net->name,
		dhd->pub.mac.octet[0], dhd->pub.mac.octet[1],
		dhd->pub.mac.octet[2], dhd->pub.mac.octet[3],
		dhd->pub.mac.octet[4], dhd->pub.mac.octet[5]);
#else
	DHD_ERROR(("Broadcom Dongle Host Driver: register interface [%s]", net->name));
#endif
